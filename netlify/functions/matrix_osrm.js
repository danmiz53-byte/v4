// netlify/functions/matrix_osrm.js
...
exports.handler = async (event) => {
  ...
  try{
    ...
    if(!res.ok) return jsonResponse(500, { error: `OSRM table HTTP ${res.status}` });
    if(!data || !data.durations) return jsonResponse(500, { error: "OSRM table missing durations" });

    return jsonResponse(200, { durations: data.durations });
  }catch(err){
